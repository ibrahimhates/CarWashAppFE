...
import { IAdditionalSalaryUpdate, ISalaryUpdate } from "./UpdateSalaryModal";
import AdditionalSalaryMembers from "./AdditionalSalaryMembers";

type Props = {
    salary: ISalaryUpdate;
    updateData: (field: Partial<ISalaryUpdate>) => void;
    currentDate: string;
}
...
const SalaryMember = ({ salary, updateData, currentDate }: Props) => {
    ...
    const [addSalaries, setAddSalaries] = useState<IAdditionalSalaryUpdate[]>([...salary.additionalSalaries]);
    ...
    const addAmount = (id: string) => {
        const updated: IAdditionalSalaryUpdate[] = [...addSalaries,
        {
            id: '',
            salaryId: salary.id,
        ...
        const select: any = document.querySelector('select[name="additionalSalaryForUpdate"]')
        select.value = ''
    }

    const updateAmount = (id: string, field: Partial<IAdditionalSalaryUpdate>) => {
        const updated: IAdditionalSalaryUpdate[] = [...addSalaries]
    ...
    const removeAmount = (id: string) => {
        const updated: IAdditionalSalaryUpdate[] = addSalaries.filter(addS => addS.additionalSalaryTypeId !== id)
        setAddSalaries(updated);
        updateData({ additionalSalaries: updated });
    }
